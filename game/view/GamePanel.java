...
public class GamePanel extends JPanel {
     ...
     * Diperbarui: Menggunakan logika akumulasi (skor = skor + baru) sesuai spesifikasi tugas.
     */
    private void saveScoreToDB(String statusGame) {
        try {
            Connection conn = DBConnection.getConnection();

            // QUERY UPDATED: Skor dan Peluru Meleset dijumlahkan (+ ?), Sisa Peluru diperbarui
            // Hal ini dilakukan agar data terus bertambah setiap kali bermain (akumulatif).
            String sql = "UPDATE tbenefit SET skor = skor + ?, peluru_meleset = peluru_meleset + ?, sisa_peluru = ?, status = ? WHERE username = ?";

            PreparedStatement pst = conn.prepareStatement(sql);
            pst.setInt(1, currentState.score);         // Menambahkan skor sesi ini ke total di DB
            pst.setInt(2, currentState.dodged);        // Menambahkan total peluru meleset sesi ini
            pst.setInt(3, currentState.ammo);          // Menyimpan sisa peluru terakhir pemain
            pst.setString(4, statusGame);              // Status ONGOING atau GAMEOVER
            pst.setString(5, this.username);

            pst.executeUpdate();
            pst.close();
            conn.close();
            System.out.println(">> DATA BERHASIL DIAKUMULASIKAN KE DATABASE.");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    protected void paintComponent(Graphics g) {
        ...
        for (com.monster.game.model.Monster m : currentState.monsters) {
            if (m.type == 1) { // Tipe Tanaman
                Image img = (m.spriteNum == 1) ? monster3 : monster4;
                if (img != null) {
                    g2.drawImage(img, (int)m.x, (int)m.y, m.width, m.height, null);
                }
            }
        }
        ...
        for (com.monster.game.model.Bullet b : currentState.bullets) {
            Image bImg;
            if (b.isPlayerBullet) {
                if (b.isPlasma) {
            ...
            if (bImg != null) {
                g2.drawImage(bImg, (int)b.x, (int)b.y, b.width, b.height, null);
            }
        }
